...
(function (global) {
  ...
  function create(definition) {
    ...
    const S = { deps: {}, root: null, canvas: null, game: null, scene: null, opening: null, closing: false, generation: 0, onClose: null };
    ...
    const open = options => {
      ...
      S.closing = false; S.onClose = typeof options?.onClose === "function" ? options.onClose : null;
      ...
      S.opening = new Promise((resolve, reject) => {
        try {
          ...
          const config = { type: global.Phaser.AUTO, parent: S.canvas, width: S.canvas.clientWidth || innerWidth, height: S.canvas.clientHeight || innerHeight, backgroundColor: "#071019", audio: { noAudio: true }, input: { activePointers: 4, touch: { capture: true } }, physics: { default: "arcade", arcade: { gravity: { x: 0, y: 0 }, debug: !!global.DBG } }, scale: { mode: global.Phaser.Scale.RESIZE }, scene: {
            create() { S.scene = this; try { definition.createScene(this, { deps: S.deps, close, definition }); this.events.once("shutdown", () => definition.cleanupSector?.(this)); resolve(true); } catch (error) { reject(error); } },

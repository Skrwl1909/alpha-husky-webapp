...
function cleanupBattleStage() {
  ...
  try { window.SiegePixi?.stop?.(); } catch (_) {}
    ...
    <div class="siege-card siege-battle-card">
      ...
      <div id="siegeBattleStage" class="siege-battle-stage">
        <div class="siege-battle-stage-empty">
          Replay ready.<br>
          <span class="siege-muted">
            In the next step this area will be rendered by Pixi.
          </span>
        </div>
      </div>
...
function ensureModal() {
 ...
 function render(raw) {
  ...
  root.innerHTML = `
    ...
    ${renderBattlePanelHTML(out, node, cur)}
  ...
  if (battlePlayBtn) {
    battlePlayBtn.onclick = () => {
      const replay = getLastReplay(out, node, cur);
      if (!replay) {

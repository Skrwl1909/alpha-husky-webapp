...
// Użycie:
//   Combat.init({ seed, feedHook: window.Dojo?.feed });
//   const hit = Combat.rollHit(playerTotals, enemyObj);   // tylko policz
//   const res = Combat.strike(playerTotals, enemyRef);    // policz + odejmij HP + (opcjonalnie) nakarm Dojo
...
(function (global) {
  ...
  function applyHit(targetRef, hit, { feed=true } = {}) {
  ...
  function strike(attackerTotals, targetRef, ctx = {}) {
    ...
    const dealt = applyHit(targetRef, hit, { feed:true });

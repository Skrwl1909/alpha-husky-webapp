...
(function () {
  ...
  async function doPatrol(nodeId) {
    ...
    try {
      ...
      if (r.archiveKeyGained) {
        const left = (r.archiveKeys && r.archiveKeys.left != null) ? r.archiveKeys.left : r.archiveKeysLeft;
        let m = "Archive Key gained. Use it in Campaign → Burned Archive";
        if (left != null) m += ` (Archive Keys: ${left})`;
        toast(m);
      } else if (r.archiveKeyCapReached) {
        toast("Archive Key cap reached");
      }

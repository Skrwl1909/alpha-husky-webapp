...
#tacticalOpsRoot .t-top {
  grid-area: top;
  z-index: 30;
  padding: max(0.4rem, env(safe-area-inset-top)) max(0.6rem, env(safe-area-inset-right)) 0.25rem
...
#tacticalOpsRoot .t-field-art {
  ...
  filter: saturate(0.78) brightness(0.62) contrast(1.1);
  width: 100%;
  height: 100%;
  min-width: 100%;
  min-height: 100%;
...
#tacticalOpsRoot .t-dock {
  grid-area: dock;
  z-index: 30;
  padding: 0.3rem max(0.55rem, env(safe-area-inset-right)) max(0.55rem, env(safe-area-inset-bottom))
...
#tacticalOpsRoot .t-act .row .slot {
  color: var(--t-faint);
  font-size: 0.58rem;
  letter-spacing: 0.14em;
}
...
#tacticalOpsRoot[data-layout="compact"] .t-status-stats {
  display: none;
}
...
/* ---- STANDARD (tablet) ---- */
#tacticalOpsRoot[data-layout="standard"] .t-brand img {
  width: 42px;
  height: 42px;
}
...
#tacticalOpsRoot[data-layout="wide"] .t-brand img {
  width: 46px;
  height: 46px;
}
...
`,Tr={requestClose:()=>{},dbg:!1};function Er(e){Tr={...Tr,...e}}function Dr(e,t){let n=Math.max(0,Math.round(e));return n<=599?`compact`:n<=899?`standard`:`wide`}var Or=`tacticalOpsRoot`,kr=`tacticalOpsStyles`,Ar=`tacticalOpsFonts`,I={apiPost:null,tg:null,dbg:!1,root:null,isOpen:!1,reactRoot:null,keyHandlerBound:!1,resizeBound:!1,prevBodyOverflow:``,prevHtmlOverflow:``,resizeObserver:null,battleWatcher:null},jr=Promise.resolve();try{window.__AH_TACTICAL_OPS_VER__=wn,window.__TACTICAL_COMBAT__=en}catch{}function Mr(){let e=document.getElementById(kr);e||(e=document.createElement(`style`),e.id=kr,document.head.appendChild(e)),e.textContent=wr}function Nr(){if(document.getElementById(Ar))return;let e=document.createElement(`link`);e.id=Ar,e.rel=`stylesheet`,e.href=`https://fonts.googleapis.com/css2?family=Rajdhani:wght@600;700&family=Source+Sans+3:wght@400;600;700&display=swap`,document.head.appendChild(e)}function Pr(){let e=I.root||document.getElementById(Or);if(!e)return;let t=window.visualViewport,n=I.tg?.viewportStableHeight||I.tg?.viewportHeight,r=Math.max(1,Math.round(t?.width||e.clientWidth||window.innerWidth)),i=Math.max(1,Math.round(t?.height||(typeof n==`number`?n:0)||window.innerHeight)),a=Math.round(t?.offsetTop||0),o=Math.round(t?.offsetLeft||0);e.style.position=`fixed`,e.style.left=`${o}px`,e.style.top=`${a}px`,e.style.right=`auto`,e.style.bottom=`auto`,e.style.width=`${r}px`,e.style.height=`${i}px`,e.style.maxHeight=`${i}px`,e.style.setProperty(`--tops-w`,`${r}px`),e.style.setProperty(`--tops-h`,`${i}px`),e.setAttribute(`data-layout`,Dr(r,i)),Fr(e,r,i)}function Fr(e,t,n){let r=e.querySelector(`.t-field-wrap`);if(!r)return;let i=e.getAttribute(`data-layout`)||Dr(t,n),a=t=>{let n=e.querySelector(t);return n?Math.round(n.getBoundingClientRect().height):0},o=a(`.t-top`)+a(`.t-order-wrap`)+a(`.t-status`)+a(`.t-dock`),s=i===`wide`?Math.max(160,n):Math.max(160,n-o);e.style.setProperty(`--tops-field-h`,`${s}px`),r.style.display=`block`,r.style.width=`100%`,r.style.minWidth=`0`,r.style.overflow=`hidden`,i===`wide`?(r.style.position=`absolute`,r.style.inset=`0`,r.style.height=`100%`,r.style.minHeight=`100%`,r.style.maxHeight=`none`):(r.style.position=`relative`,r.style.inset=`auto`,r.style.flex=`1 1 auto`,r.style.height=`${s}px`,r.style.minHeight=`${s}px`,r.style.maxHeight=`${s}px`);let c=e.querySelector(`.t-field`);c&&(c.style.position=`absolute`,c.style.inset=`0`,c.style.width=`100%`,c.style.height=`100%`);let l=e.querySelector(`.t-field-art`);l&&(l.style.position=`absolute`,l.style.inset=`0`,l.style.width=`100%`,l.style.height=`100%`,l.style.objectFit=`cover`,l.style.display=`block`)}function Ir(){Pr()}function Lr(){if(!I.resizeBound){window.addEventListener(`resize`,Ir);try{window.visualViewport?.addEventListener(`resize`,Ir),window.visualViewport?.addEventListener(`scroll`,Ir)}catch{}try{I.resizeObserver=new ResizeObserver(Ir),I.resizeObserver.observe(document.documentElement),I.root&&I.resizeObserver.observe(I.root)}catch{I.resizeObserver=null}Rr(),I.resizeBound=!0}}function Rr(){if(!(!I.root||I.battleWatcher))try{I.battleWatcher=new MutationObserver(()=>{requestAnimationFrame(()=>Pr())}),I.battleWatcher.observe(I.root,{childList:!0,subtree:!0})}catch{I.battleWatcher=null}}function zr(){if(I.resizeBound){window.removeEventListener(`resize`,Ir);try{window.visualViewport?.removeEventListener(`resize`,Ir),window.visualViewport?.removeEventListener(`scroll`,Ir)}catch{}try{I.resizeObserver?.disconnect()}catch{}I.resizeObserver=null;try{I.battleWatcher?.disconnect()}catch{}I.battleWatcher=null,I.resizeBound=!1}}function Br(){I.prevBodyOverflow=document.body.style.overflow,I.prevHtmlOverflow=document.documentElement.style.overflow,document.body.style.overflow=`hidden`,document.documentElement.style.overflow=`hidden`}function Vr(){document.body.style.overflow=I.prevBodyOverflow,document.documentElement.style.overflow=I.prevHtmlOverflow}function Hr(e){if(!I.isOpen||e.key!==`Escape`)return;let t=P.getState();if(t.screen===`battle`&&t.battle.mode===`targeting`){t.cancel(),e.preventDefault(),e.stopPropagation();return}if(t.screen===`brief`){t.backToHub(),e.preventDefault(),e.stopPropagation();return}e.preventDefault(),e.stopPropagation(),Xr()}function Ur(){I.keyHandlerBound||(document.addEventListener(`keydown`,Hr,!0),I.keyHandlerBound=!0)}function Wr(){I.keyHandlerBound&&(document.removeEventListener(`keydown`,Hr,!0),I.keyHandlerBound=!1)}function Gr(){Mr(),Nr();let e=document.getElementById(Or);return e||(e=document.createElement(`div`),e.id=Or,e.setAttribute(`data-open`,`0`),document.body.appendChild(e)),I.root=e,e}function Kr(){I.isOpen=!1,Wr(),zr(),Vr(),I.reactRoot&&(I.reactRoot.unmount(),I.reactRoot=null);let e=I.root||document.getElementById(Or);e&&(e.setAttribute(`data-open`,`0`),e.innerHTML=``);try{P.getState().backToHub()}catch{}}function qr(){let e={close:()=>{Kr()},isOpen:()=>I.isOpen},t=window;try{t.AlphaNav?.push?t.AlphaNav.push(Or,e):(t.navRegister?.(Or,e),t.navOpen?.(Or))}catch{}}function Jr(e){let t=e&&typeof e==`object`?e:{};typeof t.apiPost==`function`&&(I.apiPost=t.apiPost),t.tg&&(I.tg=t.tg),typeof t.dbg==`boolean`&&(I.dbg=t.dbg);try{if(I.apiPost&&typeof window<`u`){let e=window;typeof e.apiPost!=`function`&&(e.apiPost=I.apiPost)}}catch{}return t.onboarding===`session`?(P.getState().configureOnboarding({enabled:!0}),jr=Promise.resolve()):t.onboarding===`off`?(P.getState().configureOnboarding({enabled:!1}),jr=Promise.resolve()):jr=P.getState().loadFoundationProgression(),Er({requestClose:Xr,dbg:I.dbg}),Qr}async function Yr(e){await jr,typeof e==`string`&&e&&P.getState().openOperationBrief(e);let t=Gr();Er({requestClose:Xr,dbg:I.dbg}),I.isOpen=!0,t.setAttribute(`data-open`,`1`),Br(),Ur(),Lr(),Pr(),I.reactRoot||(I.reactRoot=(0,ue.createRoot)(t)),I.reactRoot.render((0,w.createElement)(Cr)),qr();try{I.tg?.expand?.()}catch{}requestAnimationFrame(()=>Pr()),setTimeout(()=>Pr(),50)}function Xr(){if(!I.isOpen)return;let e=window;try{if(e.AlphaNav?.close?.(Or,{source:`tactical-ops-close`}))return}catch{}Kr();try{e.navClose?.(Or)}catch{}}function Zr(){return{...er(),open:I.isOpen,version:wn}}var Qr={init:Jr,open:Yr,close:Xr,refresh:Zr,getState:Zr};function $r(){return window.TacticalOps=Qr,Qr}var ei=$r();try{let e=window;e.__AH_TACTICAL_OPS_VER__=wn,e.__TACTICAL_COMBAT__=en,e.TacticalOps=ei}catch{}return e.API=Qr,e.DEF_CURVE=me,e.GRID_COLS=de,e.GRID_ROWS=fe,e.METER_MAX=pe,e.STATUS_LABEL=Ce,e.STATUS_SHORT=we,e.VERSION=wn,e.a1Range=$t,e.advanceToNext=Wt,e.aiUsesLegalRules=st,e.applyAi=Zt,e.applySkill=$e,e.applyStatus=_e,e.availableSkills=et,e.beginUnitTurn=Ut,e.canOccupy=Pe,e.canRecover=Kt,e.cellKey=ke,e.chebyshev=Oe,e.chooseAiAction=ot,e.computeHeal=Ee,e.consumeMeter=Ge,e.cooldownLeft=Xe,e.createBattle=zt,e.default=ei,e.effectiveAtk=ye,e.effectiveDef=be,e.effectiveSpd=xe,e.evaluateOutcome=Vt,e.fieldPercent=k,e.hasStatus=ge,e.inBounds=De,e.inSkillRange=Fe,e.incomingDamageMultiplier=Se,e.isLegalTargetType=Ve,e.living=Ie,e.mitigatedDamage=Te,e.occupiedKeys=Ae,e.pickReadyId=We,e.planAi=Qt,e.previewQueue=Ke,e.reachableCells=Ne,e.resetStatusSeq=he,e.resolveSkillTargets=Re,e.skillNeedsTargetPick=Be,e.skillReady=Ze,e.startBattle=Gt,e.tickUntilReady=Ue,e.tryMove=Jt,e.tryRecover=qt,e.trySkill=Xt,e.trySkip=Yt,e.unitAt=je,e.unitSkills=Ye,e.unitsInRadius=Le,e.validTargetIds=ze,e})({});
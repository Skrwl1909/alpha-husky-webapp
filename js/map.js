...
(function () {
  ...
  // SITREP chip v1 state (map-only, non-blocking fetch of /webapp/sitrep/state)
  let _sitrepCache = null;
  let _sitrepLastFetchMs = 0;
  let _sitrepInFlight = null;
  const MAP_SITREP_STALE_MS = 45000;
...
/* === Map SITREP Chip v1 (compact, near header + world brief area; shows only when /sitrep/state ok; Archive Front not node) === */
.map-sitrep-chip{
  margin:3px 6px 2px;
  padding:5px 7px 6px;
  border:1px solid rgba(140,210,255,.18);
  border-radius:8px;
  background:linear-gradient(180deg,rgba(10,20,32,.88),rgba(6,14,24,.94));
  font-size:9px;
  line-height:1.2;
...
.map-sitrep-chip.is-visible{display:flex;}
.map-sitrep-head{display:flex;align-items:center;justify-content:space-between;gap:6px;margin-bottom:1px;flex-wrap:wrap;}
.map-sitrep-k{
  font-size:8.5px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;
  color:rgba(150,215,255,.7);background:rgba(30,80,130,.25);padding:1px 5px;border-radius:4px;
}
.map-sitrep-f{font-size:9px;font-weight:800;color:#d4eaff;letter-spacing:.01em;}
.map-sitrep-grid{display:flex;gap:6px 10px;flex-wrap:wrap;}
.map-sitrep-r{font-size:8.8px;line-height:1.15;color:rgba(195,225,255,.82);}
.map-sitrep-r span{font-weight:700;color:rgba(150,195,235,.55);margin-right:3px;}
.map-sitrep-r strong{font-weight:800;color:#d6eaff;}
.map-sitrep-r strong.is-live{color:#a8f0c7;}
.map-sitrep-r strong.is-breach{color:#f4a8a8;}
.map-sitrep-cta{
  appearance:none;align-self:flex-start;margin-top:2px;padding:3px 8px 2px;border-radius:6px;
  border:1px solid rgba(140,210,255,.22);background:rgba(35,85,125,.32);
  color:#e3f0ff;font-size:8.5px;font-weight:800;letter-spacing:.02em;cursor:pointer;
  min-height:18px;line-height:1;
}
.map-sitrep-cta:active{filter:brightness(.92);}
.map-sitrep-note{margin-top:2px;font-size:8px;color:rgba(255,210,140,.85);background:rgba(60,40,20,.3);padding:1px 4px;border-radius:3px;}
`;
  ...
  // --- Map SITREP Chip v1 helpers (fetch /webapp/sitrep/state non-blocking; defensive Burned Archive handling) ---
  function _extractSitrepChip(s) {
    if (!s || !s.ok) return null;
    const fronts = (s.fronts && typeof s.fronts === "object") ? s.fronts : null;
    const ba = fronts && fronts.burnedArchive && typeof fronts.burnedArchive === "object" ? fronts.burnedArchive : null;
    if (!ba || ba.exists === false) return null;
    const statusRaw = String(ba.status || "").toLowerCase().trim();
    let frontStatus = "INACTIVE";
    if (statusRaw === "live") frontStatus = "Live";
    else if (statusRaw === "close_to_breach" || statusRaw === "closetobreach") frontStatus = "Close to Breach";
    else if (statusRaw === "breach_confirmed" || statusRaw === "breached") frontStatus = "Breach Confirmed";
    else if (statusRaw) frontStatus = statusRaw.replace(/_/g, " ").replace(/\b\w/g, function (c) { return c.toUpperCase(); });
    const breached = !!(ba.breached === true || (ba.factionProgress && ba.factionProgress.breached));
    const breachLabel = breached ? "Confirmed" : "Not confirmed";
    let location = "Archive Front";
    const nodeRaw = String(ba.activeNodeName || ba.nodeName || ba.nodeLabel || ba.nodeId || "").trim();
    if (nodeRaw) {
      const low = nodeRaw.toLowerCase().replace(/[_ -]/g, "");
      if (low !== "burnedarchive" && low !== "burnedarch" && low !== "archivefront" && low !== "archive" && low !== "campaignfront") {
        location = nodeRaw.indexOf("_") >= 0 ? nodeRaw.replace(/_/g, " ").replace(/\b\w/g, function (c) { return c.toUpperCase(); }) : nodeRaw;
      }
    }
    return {
      ok: true,
      front: "Burned Archive",
      frontStatus: frontStatus,
      breach: breachLabel,
      location: location,
      isLive: (statusRaw === "live" || statusRaw.indexOf("breach") >= 0 || statusRaw.indexOf("close") >= 0)
    };
  }

  function _ensureMapSitrepChip() {
    const mapCard = document.querySelector("#mapBack .map-card") || document.getElementById("mapBack");
    if (!mapCard) return null;
    let chip = document.getElementById("mapSitrepChip");
    if (!chip) {
      chip = document.createElement("div");
      chip.id = "mapSitrepChip";
      chip.className = "map-sitrep-chip";
      chip.hidden = true;
      const brief = document.getElementById("mapWorldBrief");
      if (brief && brief.parentNode === mapCard) {
        brief.insertAdjacentElement("afterend", chip);
      } else {
        const head = mapCard.querySelector(".map-head");
        if (head && head.parentNode === mapCard) head.insertAdjacentElement("afterend", chip);
        else mapCard.appendChild(chip);
      }
    }
    return chip;
  }

  function _renderMapSitrepChip(data) {
    const chip = _ensureMapSitrepChip();
    if (!chip) return;
    if (!data || !data.ok) {
      chip.hidden = true;
      chip.innerHTML = "";
    ...
    const cls = data.frontStatus.indexOf("Breach") >= 0 ? "is-breach" : (data.isLive ? "is-live" : "");
    const html =
      '<div class="map-sitrep-head">' +
        '<span class="map-sitrep-k">SITREP LIVE</span>' +
        '<span class="map-sitrep-f">Front: ' + esc(data.front) + '</span>' +
      '</div>' +
      '<div class="map-sitrep-grid">' +
        '<div class="map-sitrep-r"><span>Status</span><strong class="' + cls + '">' + esc(data.frontStatus) + '</strong></div>' +
        '<div class="map-sitrep-r"><span>Breach</span><strong>' + esc(data.breach) + '</strong></div>' +
        '<div class="map-sitrep-r"><span>Loc</span><strong>' + esc(data.location) + '</strong></div>' +
      '</div>' +
      '<button type="button" class="map-sitrep-cta" data-map-sitrep-cta="report">Open Campaign Report</button>';
    chip.innerHTML = html;
    chip.hidden = false;
    chip.classList.add("is-visible");
    const btn = chip.querySelector('[data-map-sitrep-cta]');
    if (btn && !btn._sitrepCtaBound) {
      btn._sitrepCtaBound = true;
      btn.addEventListener("click", _handleMapSitrepCta, { passive: true });
    }
  }

  function _handleMapSitrepCta(ev) {
    try { ev.preventDefault(); } catch (_) {}
    const chip = document.getElementById("mapSitrepChip");
    (async () => {
      try {
        const has = !!(window.Campaign && typeof window.Campaign.open === "function");
        if (has) {
          const mapBack = document.getElementById("mapBack");
          if (mapBack) {
            mapBack.style.display = "none";
            try { if (typeof window.navClose === "function") window.navClose("mapBack"); } catch (_) {}
          }
          await window.Campaign.open();
        } else if (chip) {
          let note = chip.querySelector(".map-sitrep-note");
          if (!note) {
            note = document.createElement("div");
            note.className = "map-sitrep-note";
            chip.appendChild(note);
          }
          note.textContent = "Open Campaign from Hub.";
          setTimeout(() => { try { if (note && note.parentNode) note.parentNode.removeChild(note); } catch (_) {} }, 2400);
        }
      } catch (e) {
        try { console.warn("[AHMap] sitrep CTA failed", e); } catch (_) {}
      }
    })();
  }

  async function _loadSitrepForChip(force) {
    const now = Date.now();
    if (!force && _sitrepCache && (now - _sitrepLastFetchMs) < MAP_SITREP_STALE_MS) {
      _renderMapSitrepChip(_extractSitrepChip(_sitrepCache));
      return _sitrepCache;
    }
    if (_sitrepInFlight) {
      try { await _sitrepInFlight; } catch (_) {}
      return _sitrepCache;
    }
    const apiPost = getApiPost();
    if (!apiPost) {
      _renderMapSitrepChip(null);
      return null;
    }
    _sitrepInFlight = (async () => {
      try {
        const out = await apiPost("/webapp/sitrep/state", {});
        if (out && out.ok !== false) {
          _sitrepCache = out;
          _sitrepLastFetchMs = Date.now();
          const ex = _extractSitrepChip(out);
          _renderMapSitrepChip(ex);
          return out;
        }
        _sitrepCache = out || { ok: false };
        _renderMapSitrepChip(null);
        return _sitrepCache;
      } catch (err) {
        try { console.warn("[AHMap] /webapp/sitrep/state failed (chip hidden, map unaffected)", err); } catch (_) {}
        _renderMapSitrepChip(null);
        return null;
      } finally {
        _sitrepInFlight = null;
      }
    })();
    return _sitrepInFlight;
  }

  function _kickSitrepLoadIfMap() {
    try {
      const mapBack = document.getElementById("mapBack");
      if (!mapBack) return;
      const d = (mapBack.style && mapBack.style.display) || "";
      if (d === "none") return;
      const now = Date.now();
      if (!_sitrepCache || (now - _sitrepLastFetchMs) > MAP_SITREP_STALE_MS) {
        void _loadSitrepForChip(false);
      } else if (_sitrepCache) {
        _renderMapSitrepChip(_extractSitrepChip(_sitrepCache));
      }
    } catch (_) {}
  }
...
function _findLeaderInfoByNodeId(nodeId, leadersMap) {
  ...
  function _bindReapplyHooks() {
    ...
    document.addEventListener("click", () => {
      ...
      setTimeout(_kickSitrepLoadIfMap, 90);
    }, true);

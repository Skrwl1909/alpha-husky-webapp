...
window.Inventory = {
  async open() {
    ...
    container.innerHTML = `<div style="padding:20px;color:#fff;text-align:center;"><h2>Inventory</h2><div id="inv-loading">Loading equipment...</div></div>`;

    const res = await apiPost("/webapp/inventory/state", {});
    if (!res.ok) {
      document.getElementById("inv-loading").innerHTML = `<p style="color:#f66;">Error: ${res.reason || "Failed"}</p>`;
      return;
    }
      ...
      <div style="display:inline-block;margin:8px;width:90px;text-align:center;position:relative;">
        <img src="${item.icon || '/assets/items/unknown.webp'}" width="72" height="72"
             style="border:3px solid ${
               item.rarity==='legendary'?'#ff0':
               item.rarity==='epic'?'#a0f':
               item.rarity==='rare'?'#08f':
               item.rarity==='uncommon'?'#0f8':'#888'
             }; border-radius:12px;">
    ...
    document.getElementById("inv-loading").innerHTML = `
      <div style="margin:20px 0;font-size:16px;opacity:0.9;text-align:center;">
        Bones: <b style="color:#ff6;">${(res.bones ?? res.materials?.bones ?? 0).toLocaleString()}</b> • 
        Scrap: <b style="color:#8af;">${(res.scrap ?? res.materials?.scrap ?? 0).toLocaleString()}</b> • 
        Rune Dust: <b style="color:#f6f;">${(res.rune_dust ?? res.materials?.rune_dust ?? 0).toLocaleString()}</b>
      </div>
      ...
      <div style="text-align:center;margin-top:30px;">
        <button onclick="Telegram.WebApp.close()" style="padding:14px 36px;border-radius:16px;background:#333;border:none;color:#fff;font-size:17px;letter-spacing:0.5px;">
          ← Back to chat
        </button>
      </div>
    `;

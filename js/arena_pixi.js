// public/js/arena_pixi.js — FINAL vD-2026-02-05 (stop guessing: prefer pet_icon/pet_key; ignore emoji "icon"; try pets/ + pets/icons/)
(function (global) {
  const state = {
    apiPost: null,
    tg: null,
  ...
  const VER = "arena_pixi.js vD-2026-02-05";
  ...
  const PET_FOLDERS = ["pets", "pets/icons"]; // dopisz tu jeśli masz inny folder
  ...
  function _looksLikePathOrUrl(s) {
    ...
    // ścieżka zwykle ma / albo . (emoji i "mystic" tego nie mają)
    if (x.includes("/") || x.includes(".")) return true;
    // wersjonowanie cloudinary: v123456...
  ...
  function petAssetUrls(fighter) {
    ...
    // 1) BEZPOŚREDNI URL z backendu (najlepsze)
    const direct =
  ...
  function hpBar(app, x, y, w, h) {
    ...
    function draw(rr, X, Y, W, H) {
      gBack.clear().beginFill(0x000000, 0.35).drawRoundedRect(X, Y, W, H, 6).endFill();
      const fillW = (W - 4) * rr;
      gFill.clear().beginFill(0xffffff, 0.65).drawRoundedRect(X + 2, Y + 2, Math.max(0, fillW), H - 4, 5).endFill();
    }

    let _ratio = 1, _x = x, _y = y, _w = w, _h = h;
    draw(_ratio, _x, _y, _w, _h);

    return {
      set(ratio) { _ratio = Math.max(0, Math.min(1, ratio)); draw(_ratio, _x, _y, _w, _h); },
      setPos(x2, y2, w2, h2) { _x = x2; _y = y2; _w = w2; _h = h2; draw(_ratio, _x, _y, _w, _h); }
    };
  }
  ...
  async function renderReplayPixi(stub) {
    ...
    app.stage.addChild(tLeft); app.stage.addChild(tRight);
    ...
    app.stage.addChild(tHpL); app.stage.addChild(tHpR);
    ...
    const leftF = await (async () => {
      const c = new global.PIXI.Container(); app.stage.addChild(c);
      const urls = petAssetUrls(left);
      const tex = await loadTextureSafeMany(urls);
      if (state.dbg) log("pet urls LEFT", { pet_name: left.pet_name, pet_key: left.pet_key, pet_type: left.pet_type, urls, ok: !!tex });

      let obj;
      if (tex) {
        obj = new global.PIXI.Sprite(tex);
        obj.anchor?.set?.(0.5);
        obj.scale.set(0.60);
      } else {
        obj = new global.PIXI.Text("🐾", new global.PIXI.TextStyle({ fill: 0xffffff, fontSize: 56, fontWeight: "900" }));
        obj.anchor?.set?.(0.5);
      }
      c.addChild(obj);

      const badge = new global.PIXI.Text(pickPetLabel(left), subStyle);
      badge.x = -badge.width / 2; badge.y = 52;
      c.addChild(badge);
      return c;
    })();

    const rightF = await (async () => {
      const c = new global.PIXI.Container(); app.stage.addChild(c);
      const urls = petAssetUrls(right);
      const tex = await loadTextureSafeMany(urls);
      if (state.dbg) log("pet urls RIGHT", { pet_name: right.pet_name, pet_key: right.pet_key, pet_type: right.pet_type, urls, ok: !!tex });
      ...
      if (tex) {
        ...
        obj.anchor?.set?.(0.5);
        obj.scale.set(0.60);
        obj.scale.x = -Math.abs(obj.scale.x); // mirror only sprite
      } else {
        obj = new global.PIXI.Text("🐾", new global.PIXI.TextStyle({ fill: 0xffffff, fontSize: 56, fontWeight: "900" }));
        obj.anchor?.set?.(0.5);
      }
      ...
      const badge = new global.PIXI.Text(pickPetLabel(right), subStyle);
      badge.x = -badge.width / 2; badge.y = 52;
      c.addChild(badge);
      return c;
    })();
    ...
    async function punch(node, dx) { const start = node.x; node.x = start + dx; await sleep(90); node.x = start; }
    async function hitFlash(node) { const sy = node.scale.y; node.scale.y = sy * 0.92; await sleep(70); node.scale.y = sy; }
    ...
    function dmgFloat(x, y, text) {
      ...
      const tick = (dt) => {
        ...
        if (life >= dur) { app.ticker.remove(tick); try { t.destroy(); } catch (_) {} }
      };
      app.ticker.add(tick);
    }
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      if (dodged) { dmgFloat(target.x - 10, target.y - 90, "DODGE"); await sleep(260); continue; }

      const hit = dmg + glitch;
      if (hit > 0) { await hitFlash(target); dmgFloat(target.x - 10, target.y - 90, (crit ? "CRIT " : "") + "-" + hit); }

      const pHp = parseInt(s.pHp ?? s.playerHp ?? s.youHp ?? leftHp, 10);
      const eHp = parseInt(s.eHp ?? s.enemyHp ?? s.oppHp ?? rightHp, 10);

      if (youAreP1) { leftHp = isFinite(pHp) ? pHp : leftHp; rightHp = isFinite(eHp) ? eHp : rightHp; }
      else { leftHp = isFinite(eHp) ? eHp : leftHp; rightHp = isFinite(pHp) ? pHp : rightHp; }

      if (!isFinite(pHp) || !isFinite(eHp)) {
        if (dotSelf > 0) { if (attackerLeft) leftHp -= dotSelf; else rightHp -= dotSelf; }
        if (hit > 0) { if (attackerLeft) rightHp -= hit; else leftHp -= hit; }
      }
  ...
  async function open(battleId) {
    ...
    const doLoad = async () => {
      ...
      try { global.__ARENA_LAST_STUB__ = stub; global.__ARENA_LAST_BATTLE_ID__ = state.lastBattleId; } catch (_) {}
